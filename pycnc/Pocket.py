...
class Pocket(DepthOp):
    ...
    def AppendTextToProgram(self):
        tool = HeeksCNC.heekscnc.program.tools.FindTool(self.tool_number)
        if tool == None:
        ...
        HeeksCNC.heekscnc.cad.WriteAreaToProgram(self.sketches)
        HeeksCNC.heekscnc.program.python_program += "entry_moves = []\n"

        # reorder the area, the outside curves must be made anti-clockwise and the insides clockwise
        HeeksCNC.heekscnc.program.python_program += "a.Reorder()\n"
        ...
        # Pocket the area
        HeeksCNC.heekscnc.program.python_program += "area_funcs.pocket(a, tool_diameter/2, "
        HeeksCNC.heekscnc.program.python_program += str(self.material_allowance / HeeksCNC.heekscnc.program.units)
        HeeksCNC.heekscnc.program.python_program += ", rapid_safety_space, start_depth, final_depth, "
        HeeksCNC.heekscnc.program.python_program += str(self.step_over / HeeksCNC.heekscnc.program.units)
        HeeksCNC.heekscnc.program.python_program += ", step_down, clearance"
        HeeksCNC.heekscnc.program.python_program += (", True" if self.starting_place else ", False")
        HeeksCNC.heekscnc.program.python_program += (", True" if self.keep_tool_down_if_poss else ", False")
        HeeksCNC.heekscnc.program.python_program += (", True" if self.use_zig_zag else ", False")
        HeeksCNC.heekscnc.program.python_program += ", " + str(self.zig_angle)
        HeeksCNC.heekscnc.program.python_program += ")\n"

        # rapid back up to clearance plane
        HeeksCNC.heekscnc.program.python_program += "rapid(z = clearance)\n"
